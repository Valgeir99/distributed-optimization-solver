...
#schedule.every(5).minutes.do(update_problem_instance_status)   # not necessarry but this will clean up solver memory if problem instance is not active anymore NOTE: we assume reward is "infinite" so problem instance will never be inactive

# TODO: to not schedule validate soltions like this we need to have some randomness otherwise all agents are basically started at the same time 
# and they will all validate the solution at the same time which is not good - overloads central node and just does not emulate real world scenario

# Download a couple of problem instances at the beginning
for i in range(5):
    download_problem_instance()
...
try:
    while elapsed_time < execution_time:
        ...
        if agent.solving_problem_instance_name is None:
            ...
            # TODO: we probably also don't want this behaviour since we don't want all agents all of a sudden to start solving the same new problem instance that just came to the platform 
            # This is of course not applicable in our experiments since we never have a new problem instance...
            # if problem_instance is not None:
            #     print("Solve new problem instance after downloading it")
            ...
            # No new problem instance to solve - so solve an active problem instance we already have downloaded (random)
            names = list(agent.problem_instances.keys())
            random.shuffle(names)
            for name in names:
                if agent.problem_instances[name]["active"]:
                    print("Solve problem instance that is already downloaded")
                    solver_thread = solve_problem_instance(name)
                    break

        # Validate with 1/6 probability (since we sleep 10 seconds so it is like we would validate every minute)
...
###
# TODO: instead of having a solver thread we could just use a single thread for all actions. This way we can kind of control more how long time the agent is solving 
# problems and how long time it is validating solutions, like Joe talked about! (from the network of agent perspective this might more suitable, just mention that in the 
# report that of course it is easy to offload the solving to a separate thread but that is not the point of this experiment)
###